    ...
    function () {
      async function fetchMovies() {
        try {
          ...
          // Resetting error before api call
          setError("");
          ...
          if (!res.ok)
            throw new Error(
              `Status Code: ${res.status}, something went wrong fetching movies`,
            );

          const data = await res.json();

          // When query search result dont found anything show this error
      ...
      // If this condition are not fulfilled, it won't run fetchMovies() function
      if (query.length < 3) {
        setMovies([]);
        setError("");
        return;
    ...
    <>
      ...
      <Main>
        ...
        <Box>
          ...
          {/* Error message will show when internet connection will be lose */}
